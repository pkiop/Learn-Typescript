...
import {ReducerActions, START_GAME, OPEN_CELL, INCREMENT_TIMER, NORMALIZE_CELL,
        QUESTION_CELL, CLICK_MINE, FLAG_CELL} from './action';
...
const plantMine = (row: number, cell:number, mine:number): (typeof CODE[keyof typeof CODE])[][] => {
  ...
  const data: (typeof CODE[keyof typeof CODE])[][] = [];
  for(let i=0;i<row;++i) {
    const rowData: (typeof CODE[keyof typeof CODE])[] = [];
    data.push(rowData);
